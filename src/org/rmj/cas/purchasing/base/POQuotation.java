...
public class POQuotation implements GTransaction{
    @Override
    public UnitPOQuotationMaster newTransaction() {
        UnitPOQuotationMaster loObj = new UnitPOQuotationMaster();
    ...
    @Override
    public UnitPOQuotationMaster loadTransaction(String fsTransNox) {
        UnitPOQuotationMaster loObject = new UnitPOQuotationMaster();
    ...
    @Override
    public UnitPOQuotationMaster saveUpdate(Object foEntity, String fsTransNox) {
        String lsSQL = "";
        
        UnitPOQuotationMaster loOldEnt = null;
        UnitPOQuotationMaster loNewEnt = null;
        UnitPOQuotationMaster loResult = null;
        
        // Check for the value of foEntity
        if (!(foEntity instanceof UnitPOQuotationMaster)) {
            setErrMsg("Invalid Entity Passed as Parameter");
            return loResult;
        }
        
        // Typecast the Entity to this object
        loNewEnt = (UnitPOQuotationMaster) foEntity;
        ...
        if (fsTransNox.equals("")){
            try {
                ...
                Logger.getLogger(POQuotation.class.getName()).log(Level.SEVERE, null, ex);
            }
        }else{
            try {
    ...
    public boolean deleteTransaction(String fsTransNox) {
        UnitPOQuotationMaster loObject = loadTransaction(fsTransNox);
    ...
    public boolean addDetail() {
        UnitPOQuotationDetail loDetail = new UnitPOQuotationDetail();
    ...
    private boolean saveDetail(String fsTransNox, boolean fbNewRecord) throws SQLException{
        ...
        UnitPOQuotationDetail loDetail;
        UnitPOQuotationDetail loOldDet;
        int lnCtr;
        String lsSQL;
    ...
    private UnitPOQuotationDetail loadTransDetail(String fsTransNox, int fnEntryNox) throws SQLException{
        UnitPOQuotationDetail loObj = null;
        ...
        if (!loRS.next()){
            ...
            //load each column to the entity
            loObj = new UnitPOQuotationDetail();
            for(int lnCol=1; lnCol<=loRS.getMetaData().getColumnCount(); lnCol++){
    ...
    private ArrayList<UnitPOQuotationDetail> loadTransDetail(String fsTransNox) throws SQLException{
        UnitPOQuotationDetail loOcc = null;
        Connection loConn = null;
        loConn = setConnection();
        
        ArrayList<UnitPOQuotationDetail> loDetail = new ArrayList<>();
        ResultSet loRS = poGRider.executeQuery(
                            MiscUtil.addCondition(getSQ_Detail(), 
                                                    "sTransNox = " + SQLUtil.toSQL(fsTransNox)));
        ...
        loRS.first();
        while(loRS.next()){
            loOcc = new UnitPOQuotationDetail();
            //load each column to the entity
            for(int lnCol=1; lnCol<=loRS.getMetaData().getColumnCount(); lnCol++){
    ...
    private ArrayList<UnitPOQuotationDetail> poDetail;
    private final String pxeMasTable = "PO_Quotation_Master";
    private final String pxeDetTable = "PO_Quotation_Detail";
}